...
const Invoice = () => {
  ...
  // Tangkap data dari halaman PaymentGateway
  const { items, subtotal, ongkir, total, customer, orderId, status } = location.state || {};

  useEffect(() => {
    window.scrollTo(0, 0);
    // Jika tidak ada data (user mencoba akses URL langsung), usir ke home
    if (!orderId) navigate("/");
  }, [orderId, navigate]);
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-white pt-24 pb-12 px-4 flex justify-center items-center">
      <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="bg-white text-black w-full max-w-md rounded-xl p-8 shadow-[0_0_60px_rgba(6,182,212,0.4)] relative">
        ...
        <div className="border-t-2 border-dashed border-gray-200 my-6 pt-6 text-center">
          <p className="text-[10px] text-gray-500 italic leading-relaxed">
            Pesanan Anda sedang diproses. Kami akan mengonfirmasi pembayaran Anda dalam waktu 1x24 jam. Terima kasih telah memilih RoadFix AI!
          </p>
