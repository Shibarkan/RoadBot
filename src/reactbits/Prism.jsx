...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const dpr = Math.min(1.5, window.devicePixelRatio || 1) * resolutionScale;
    ...
    const resize = () => {
      ...
      program.uniforms.uOffsetPx.value = [offX * dpr, offY * dpr];
    ...
    let wX = (0.3 + rnd() * 0.6) * RSX;
    let wY = (0.2 + rnd() * 0.7) * RSY;
    let wZ = (0.1 + rnd() * 0.5) * RSZ;

    let phX = rnd() * Math.PI * 2;
    let phZ = rnd() * Math.PI * 2;
