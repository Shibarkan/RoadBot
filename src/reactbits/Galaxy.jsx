...
float trisn(float x) {
  return 2.0 *
    (1.0 - smoothstep(0.0, 1.0, abs(2.0 * fract(x) - 1.0))) - 1.0;
}
...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      float hue =
        atan(base.g - base.r, base.b - base.r)
        / 6.2831853 + 0.5;
      ...
      float star =
        Star(gv - offset - pad, flareSize);

      float twinkle =
        trisn(uTime * uSpeed + seed * 6.2831)
        * 0.5 + 1.0;
...
void main() {
  ...
  if (uAutoCenterRepulsion > 0.0) {
    ...
    vec2 repulsion =
      normalize(uv)
      * (uAutoCenterRepulsion / (centerDist + 0.1));
    ...
    vec2 mousePosUV =
      (uMouse * uResolution.xy - focalPx)
      / uResolution.y;

    float mouseDist = length(uv - mousePosUV);

    vec2 repulsion =
      normalize(uv - mousePosUV)
      * (uRepulsionStrength / (mouseDist + 0.1));
  ...
  for (
    ...
    float fade =
      depth * smoothstep(1.0, 0.9, depth);

    col +=
      StarLayer(uv * scale + i * 453.32)
      * fade;
  }
...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      const width =
        ctn.offsetWidth * resolutionScale;

      const height =
        ctn.offsetHeight * resolutionScale;
  ...
  useEffect(() => {
    ...
    u.uMouseRepulsion.value =
      mouseRepulsion;

    u.uTwinkleIntensity.value =
      twinkleIntensity;

    u.uRotationSpeed.value =
      rotationSpeed;

    u.uRepulsionStrength.value =
      repulsionStrength;

    u.uAutoCenterRepulsion.value =
      autoCenterRepulsion;
  }, [
    focal,
    rotation,
